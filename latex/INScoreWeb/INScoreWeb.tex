...
Audio objects are \emph{connectable} objects i.e. objects which output can be connected to the input of another audio object. In the INScore model, the following objects are audio objects: \OSC{audio}, \OSC{video}, \OSC{faust} (see section \ref{faustObjects}) and \OSC{audioio} (see section \ref{audioioObjects})  \inprogress{not yet implemented for \OSC{audio} and \OSC{video} types}.

Audio objects support the following messages  \inprogress{not yet implemented)}:
...
\begin{rail}
remoteHost : (([1] 'http://' |  [2] 'ws://') hostname ':' portnum) +
\end{rail}
...
Although the native version of INScore supports this format for the \OSC{http} and \OSC{ws} protocols, nothing prohibits another application to control INScore web pages, provided that the format described above is respected.

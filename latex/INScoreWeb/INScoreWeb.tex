...
\subsublevel{MIDI verbose mode}
\label{verbMIDI}

In order to facilitate the detection of messages sent by a MIDI interface, a 'verbose' mode allows incoming messages to be displayed on the browser console. Real-time and system exclusive messages are filtered out.
...
The message parameter is a boolean value.
...
\begin{itemize}
\item \OSC{connect}: connect the outputs of the object to the destination inputs. \OSC{destination} must be another audio object. Without  numeric prefix and suffix [1], inputs and outputs count must be the same on source and destination. The second form [2] connects only the corresponding output channel to the corresponding input (indexed from 1). The destination supports regular expressions.

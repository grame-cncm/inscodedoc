...
\item \OSC{connect}: connect the ouputs of the Faust object to the destination inputs. The destination must be a web audio node (typically another faust object). Inputs and outputs count must be the same on source and destination. \inprogress{not yet implemented}
\end{itemize}

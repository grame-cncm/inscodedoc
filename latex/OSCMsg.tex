...
\begin{itemize}
\item \OSC{sw} refers to the scene width.
\item \OSC{sh} refers to the scene height.
\end{itemize}

\example \\
Putting on object on top of the scene (whatever its dimension):
\sample{/ITL/scene/myObject x -1sh;}
...
dateQuery : 'get' (
			...
			| ('date\%a'))
\end{rail}
\index{Time messages!absolute!date\%f}
\index{Time messages!absolute!date\%a}

\begin{itemize}
\item \OSC{date}: gives the object date as a rational value.
\item \OSC{date\%f}: gives the object date as a float value.
\item \OSC{date\%a}: gives the object date as an absolute time. It assumes that the tempo is 60bpm.
\end{itemize}
...
Images, video and audio are supported using various formats. See section \fullref{fileset} for more details on the supported formats.

\index{Set type!img}
\index{Set type!memimg}
\index{Set type!video}
\index{Set type!audio}
...
\begin{itemize}
\item \OSC{img}: an image file. The image format is inferred from the file extension.
...
\item \OSC{video}: a video file. The video format is inferred from the file extension. Note that navigation through the video is made using its \OSC{date}.
\item \OSC{audio}: an audio file. The audio format is inferred from the file extension. \OSC{audio} objects appear as a waveform but this waveforom is not (yet) computed from the audio data but randomly generated.
\end{itemize}
...
\begin{itemize}
\item \OSC{file}: a generic type to handle file based objects. Actually, the \OSC{file} type is translated into a one of the \OSC{txtf}, \OSC{gmnf}, \OSC{img}, \OSC{audio} or \OSC{video} types, according to the file extension (see table \ref{fileTranslate}).
\end{itemize}
...
\OSC{.mp3 .wav .aiff .aif, .m4a, .aac} & \OSC{audio} \\
...
\OSC{width} and \OSC{height} messages are accepted by the almost all components except textual components (see section \ref{textscore}).

\index{Specific messages! width}
\index{Specific messages! height}

\begin{rail}
widthMsg :  'width' dimension
			| 'height' dimension
\end{rail}

\begin{rail}
dimension :  (float32 ( [1] | [2] ('sw' | 'sh'))) 
\end{rail}

\begin{itemize}
\item \OSC{1}: a single float value express the dimension in the inscore coordinate system.
\item \OSC{2}: a float value suffixed with \OSC{sw} or \OSC{sh} express a dimension relative to the actual scene dimension. \OSC{sw} stands for 'scene width' and \OSC{sh} for 'scene height'. Such dimension dynamically adjusted to any change in the scene dimension.
\end{itemize}

\example
\sample{/ITL/scene/myObject width 1; \hspace{1.2cm}\icomment\  using the inscore coordinate system \\
/ITL/scene/myObject width 0.9sw;     \hspace{0.55cm}\icomment\  using the scene width 
}
...
Media objects (\OSC{audio} \OSC{video}) have their own internal time and duration that is independent from the INScore time and duration. 
...
\index{Specific messages! audio! play}
\index{Specific messages! audio! volume}
\index{Specific messages! audio! rate}
\index{Specific messages! audio! vdate}
...
\begin{itemize}
\item \OSC{play} start or stop playing the media. Default value is 0. 
\item \OSC{volume} sets the audio volume. Default and maximum value is 1.
\item \OSC{rate} sets the media playing rate. Default value is 1.
\item \OSC{vdate} sets the current video frame or audio position. Default value is 0. Arguments are the following:
\begin{itemize}
...
\index{Specific messages! audio! vduration}
...
\begin{itemize}
\item \OSC{mls} gives the media absolute duration in milliseconds.
\item \OSC{vduration} gives the media duration in musical time. The returned value is a rational computed using the current rate, according to a tempo value of 60.
\end{itemize}

A media object supports specific events (see section \fullref{typespecevents} for more details). 
\\
...
A specific \OSC{text} query is provided to get the textual content.

\example
\sample{/ITL/scene/text get text;
}
...
\index{ITL messages!quit}
\index{ITL messages!scenes}
...
\index{ITL messages!errorAddress}
...
\item \OSC{scenes}: creates a set of scenes. Arguments is a list of scene names. 
...
\item \OSC{errorAddress}: set the OSC address of error messages. Default is \OSC{error:} (which is not a valid OSC address but preserved for compatibility reasons).
...
\sample{/ITL 192.168.0.5  7000 7001 7002}
...
The application supports the \OSC{get} messages for its parameters (see section \fullref{getsect}). In addition, it provides the following messages to query version numbers  or IP address.

\index{ITL messages!version}
\index{ITL messages!guido-version}
\index{ITL messages!musicxml-version}
\index{ITL messages!IP}
\index{ITL messages!scount}

\begin{rail}
ITLRequest : 'get'  ('version' | 'guido-version' | 'musicxml-version' | 'IP' | 'scount')
\end{rail}
...
\item \OSC{IP}: the host IP address.
\item \OSC{scount}: gives the count of scenes.
...
\sample{/ITL version \inscoreversion}
...
The application level provides the static nodes \OSC{stats}, \OSC{debug}, \OSC{log} and \OSC{ssl}, available at \OSC{/ITL/stats} \OSC{/ITL/debug}, \OSC{/ITL/log} and \OSC{/ITL/ssl}. 
...
The \OSC{ssl} node is provided to manage certificates for encrypted communication with remote hosts. See section \fullref{SSL} for more details and section \fullref{Forwarding} for communication scheme.
...
\index{Scene query!swidth}
\index{Scene query!sheight}
...
\item \OSC{swidth}: gives the scene width from objects viewpoint (see the note about scene dimensions below).
\item \OSC{sheight}: gives the scene height from objects viewpoint (see the note about scene dimensions below).
\end{itemize}
...
\note{About the scene dimensions} \\
Similarly to any object, the scene width and height (set using the \OSC{width} and \OSC{height} messages) are expressed regarding its parent (i.e. the screen). The \OSC{swidth} and \OSC{sheight} messages give the scene dimensions as seen by embedded objects. They are provided for convenience since the returned values can be computed from the current width and height.

\example \\
Fill a scene with a rectangle:
\sample{/ITL/scene/r set rect \$(/ITL/scene get swidth) \$(/ITL/scene get sheight);
}
...
When you use \OSC{https} as a communication protocol, a digital certificate and an encryption key are required to establish connections between INScore and the client stations. The certificate must be signed by a trusted certificate authority. Short term certificates are freely available on the internet. 

Before issuing a \OSC{forward} message on an encrypted communication channel, the key and certificate must be loaded by INScore. For this purpose, a set of messages is provided by the application ssl node  \OSC{/ITL/ssl}:

\index{Forwarding!key}
\index{Forwarding!cert}
\index{Forwarding!cacert}
...
\begin{itemize}
\item \OSC{key} load 'file' as private key 
\item \OSC{cert} load 'file' as digital certificate
\item \OSC{cacert} load 'file' as intermediate certificate used as certificate authority 
\end{itemize}

\emph{file} is a path to a file that can be absolute or relative. 
Resolution of relative paths differs from the approach used with \OSC{rootPath}: a path expressed as relative will always refer to the \OSC{.ssh} folder located at the root of the user directory (\$HOME).
...
\index{Forwarding!accept}
\index{Forwarding!reject}
...
Messages with address starting with \OSC{/ITL/scene/line} or addressed to \OSC{/ITL/scene/rect} are filtered only if message address is not \OSC{/ITL/scene/line2} or if the content is not \OSC{scale} or \OSC{arrows}.\\
\\
...
\note{}\\
INScore is message based in a consistent way: all actions are triggered by messages (including, for example, quitting the application). Some messages are generated internally. This is the case of \OSC{ddate}: an object sends \OSC{ddate} messages to itself when its tempo is not zero. It is therefore recommended to filter \OSC{ddate}: in the absence of a filter, a recipient who receives a \OSC{tempo} message will generate these messages internally, but will also receive those that have been forwarded. Its actual tempo will then be twice the one set.

%===============================
%:  Specific queries
\sublevel{Specific queries}
\label{ITLForwardQueries}

When using connection oriented protocols with the \OSC{forward} message (\OSC{http}, \OSC{https}, \OSC{ws}), you can query the number of connected clients:

\index{Forwarding!get}

\begin{rail}
ITLClientsQuery : 'get' 'clients'
\end{rail}
The message must be addressed at the application level (\OSC{/ITL}). 
The answer is detailed for each type of connection-oriented protocol in service.
...
\index{Interaction!Events!keyDown}
\index{Interaction!Events!keyUp}

\begin{rail}
keyEvent : 	('keyDown' | 'keyUp') ([1] char | [2] '[' charlist ']' | [3] '*' | [4] controlKey)
\end{rail}

\index{Interaction!Events!controlKey}
\begin{rail}
controlKey : 'ArrowLeft' | 'ArrowRight' | 'ArrowUp' | 'ArrowDown'
\end{rail}

\begin{itemize}
\item 1) only the specified \OSC{char} will trigger the event,
\item 2) \OSC{charlist} is a string interpreted like the bracket expression of regular expressions. For example, using \OSC{[abc]}, 'a' or 'b' or 'c' will trigger a key event, using \OSC{[1-9]}, any numeric from 1 to 9 will trigger a key event.
\item 3) any key triggers a key event.
\item 4) handles special keys to trigger the key event. A literal description of the key must be provided (e.g. \OSC{ArrowLeft}).
\end{itemize}

\note\\
Unlike standard applications, there is no notion of focus regarding key events: when several objects request keyboard input, all of them will receive \OSC{key} events, provided they are watching the same input.
...
\item the \OSC{newElement} event is supported at scene and application (\OSC{/ITL}) level and triggered when a new element is added to the scene or when a new scene is created.
...
\item the \OSC{ready} event is supported by media objects (\OSC{audio} and \OSC{video}). It is triggered when media data (duration, graphic dimensions) are available.
\item the \OSC{end} event is supported by media objects (\OSC{audio} and \OSC{video}). It is triggered when a media is playing and reaches the end of the media. In this case, the object \OSC{play} status is automatically switched to 0 to reflect the actual player state.
\end{itemize}
...
Although not an object attribute, the \OSC{del} message can be watched and triggered when an object is deleted.
...
\item \OSC{\%f} or \OSC{\%a}: request the date value as a float value or as an absolute time value. Absolute time is expressed in seconds, assuming a tempo value of 60 bpm. Actually, \$date\%a = \$date\%f / 4
\end{itemize}

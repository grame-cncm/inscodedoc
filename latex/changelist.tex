...
\begin{itemize}
\item INScore Web version
\item new \OSC{faust} object (web version)
\item new \OSC{connect} method (web version)
\item forwarding mechanism extended with http and ws support. See section \fullref{ITLForward}
\end{itemize}
...
\item environement variables introduced in scripting environment (OSName and OSId). See section \fullref{envvar}.
\item new math expressions introduced in scripting context. See section \fullref{mathExpr}.
% Version 1.20
...
\item new javascript \OSC{osname} function that gives the current operating system name. See section \fullref{jsobj}.
\item new javascript \OSC{osid} function that gives the current operating system as an id. See section \fullref{jsobj}.

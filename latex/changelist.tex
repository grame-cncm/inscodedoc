...
%===============================
%:    Differences to version 1.31  (version 1.34)
\sublevel{Version 1.34 vs version 1.31}
\begin{itemize}
\item new \OSC{scount} query at application level (\OSC{/ITL}): gives the scenes count. See section \fullref{ITLQuery}
\item new \OSC{scenes} message at application level (\OSC{/ITL}). See section \fullref{applmgmt}
\item new \OSC{errorAddress} message at application level (\OSC{/ITL}): set the OSC address of error messages. See section \fullref{applmgmt}
...
\item add support for get 'date\%a' query: gives the absolute date in seconds (assumes the tempo is 60 bpm).
